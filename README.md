...
15. 超过 1 分钟的工作，必须严格分成两步：第一步是分析阶段，第二步是修改阶段；禁止混在一起执行。
16. 分析阶段只能做“为理解任务所必需的最小分析”：读取必要文件、确认改动范围、识别依赖关系、整理执行计划；禁止在这个阶段修改任何代码、文案、配置、数据库数据。
17. 分析阶段输出必须一次性讲清楚：用户要什么、我准备改哪里、哪些内容不会动、准备怎么改；说完后必须立刻停止，等待用户明确回复“改 / 开始改 / 执行”之类的指令。
18. 在用户没有明确回复“改”之前，禁止提前进入修改阶段；禁止“先顺手改一点”“先改一半试试”“边改边想”。
19. 只要用户已经明确回复“改”，就必须立刻进入修改阶段；禁止继续补充新的理解、计划、重复确认、重复分析，禁止再做没有意义的文件阅读和结构排查。
20. 修改阶段允许的“查看”只限于与落代码直接相关的最小查看，例如打开将要修改的具体文件、定位具体代码位置、查看紧邻上下文；这不属于重新分析，禁止借此重新进入长时间分析。
21. 修改阶段只能做与当前已确认范围直接相关的改动；如果遇到真正阻塞（例如权限不足、文件冲突、需求自相矛盾、缺失关键输入），才允许暂停并说明阻塞；除真实阻塞外，必须持续修改直到完成。
22. 该分析时就只分析，不准改；该修改时就只修改，不准做没有意义的分析；一旦进入修改阶段，默认以执行为主，不得再次退回分析循环。
