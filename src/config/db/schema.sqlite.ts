...
// 风控相关表 - 用于IP登录次数限制
export const loginAttempt = table(
  'login_attempt',
  {
    id: text('id').primaryKey(),
    ipAddress: text('ip_address').notNull(),
    fingerprint: text('fingerprint'), // 浏览器指纹
    userId: text('user_id').references(() => user.id, { onDelete: 'set null' }),
    attemptTime: integer('attempt_time', { mode: 'timestamp_ms' })
      .default(sqliteNowMs)
      .notNull(),
    success: integer('success', { mode: 'boolean' }).notNull(), // 是否登录成功
    userAgent: text('user_agent'),
    createdAt: integer('created_at', { mode: 'timestamp_ms' })
      .default(sqliteNowMs)
      .notNull(),
  ...
  (table) => [
    // 按IP地址查询登录尝试
    index('idx_login_attempt_ip').on(table.ipAddress),
    // 查询特定时间段内的登录尝试
    index('idx_login_attempt_time').on(table.attemptTime),
    // 复合索引：IP + 时间段，用于风控查询
    index('idx_login_attempt_ip_time').on(table.ipAddress, table.attemptTime),
  ]
);

// IP黑名单表
export const ipBlacklist = table(
  'ip_blacklist',
  {
    id: text('id').primaryKey(),
  ...
  (table) => [
    index('idx_ip_blacklist_ip').on(table.ipAddress),
    index('idx_ip_blacklist_blocked_until').on(table.blockedUntil),
  ]
);
